...
`AeatResponseFixtures.response` supports accepted, accepted-with-errors, rejected, duplicate, and wait-value scenarios. `soapFault()` returns a synthetic HTTP 500 fault. Script `NetworkFailure`, `Timeout`, `NonXmlResponse`, and `ResponseTooLarge` directly for transport branches.

Response fixtures are checked against the archived response XSD. Supply the invoice identity and `AeatOperationType` to match the submitted record. Their documents and descriptions are synthetic; accepted-with-errors uses code `2000` and rejection uses `3002` to exercise the archived official catalogue mappings. Warning-only responses are partially accepted; fully rejected/duplicate responses omit the new CSV. See the [fixture tests](../verifactu-testkit/src/commonTest/kotlin/dev/verifactu/testkit/VerifactuFixturesTest.kt) and [offline examples](../samples/offline/README.md).
