# Developer Documentation

This directory contains developer-facing documentation for VeriFactu KMP.

The README stays short. Product scope, integration details, and compliance explanations live here.

## Documents

- [Getting started](getting-started.md)
- [Core concepts](core-concepts.md)
- [Integration flow](integration-flow.md)
- [Protocol behavior](protocol-behavior.md)
- [Error handling](error-handling.md)
- [Integration responsibilities](integration-responsibilities.md)
- [Compliance sources](compliance/sources.md)
- [Compliance traceability](compliance/README.md)

## Documentation Rules

- Use English for all project documentation.
- Preserve official Spanish terms where translation would reduce protocol clarity.
- Treat BOE and AEAT materials as normative sources.
- Keep examples aligned with the public API once implementation exists.
- Do not present planned APIs as released APIs.
- Update documentation when public behavior, compliance baseline, or integration responsibility changes.
