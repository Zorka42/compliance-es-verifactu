...
- [App-accounting contract](src/commonMain/kotlin/dev/verifactu/sample/AppAccountingAdapterContract.kt) and [its test](src/commonTest/kotlin/dev/verifactu/sample/AppAccountingAdapterContractTest.kt): application-owned finalization mapping, atomic record-plus-head persistence boundary, and saved-request replay through a fake transport.
...
The sample prints only synthetic hashes/statuses/counts. Raw record XML and QR data remain in the returned example artifacts for tests. Its app-accounting contract uses a fake host store to verify atomic record-plus-chain-head persistence and saved-request replay; it is not a database implementation. A real host owns durable storage, per-chain concurrency, queues, credentials, and delivery recovery, and must address the library's documented validation gaps before production use.
