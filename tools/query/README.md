...
The tool validates with the selected JAXP provider. Some providers count supplementary characters as two UTF-16 units for XSD length facets, although the specification counts characters. The boundary test probes the provider with a synthetic one-character schema and verifies the same behavior against the archived AEAT schema for both query roles. Providers with this limitation conservatively reject names near the supplementary-character boundary; the tool does not truncate text or change global JVM settings. Common fiscal validation retains the specification's character counts. Local schema validation is not evidence of live AEAT acceptance.
